...
def _post_admin_action(client, action, ids):
    """Выполнить административное действие для выбранных документов."""
...
def test_admin_approve_sends_email_to_user(client, admin_user, user, db):
    """Проверяет, что при одобрении документа отправляется письмо пользователю."""
...
def test_admin_reject_sends_email_to_user(client, admin_user, user, db):
    """Проверяет, что при отклонении документа отправляется письмо пользователю."""
    client.force_login(admin_user)
